...
# pos.environ['CUDA_VISIBLE_DEVICES'] = "0"
...
def test():
    ...
    f_output = open('..//val_output_coco_cnn.json', 'w')
    json.dump(result_obj, f_output, indent=4)
    f_output.close()
    f_ann = open('..//val_annotation_coco_cnn.json', 'w')
    json.dump(annotation_obj, f_ann, indent=4)
    f_ann.close()

    # os.system('{} \"{}\" {} {}'.format('/home/mcislab3d/miniconda2/bin/python', '/media/mcislab3d/Seagate Backup Plus Drive/zwt/coco-caption-master/eval.py', '..//val_annotation_coco.json', '..//val_output_coco.json'))


def test1(lm_cnn, global_step):
    annotation_file_name = '../all_results/results_cnn_chn_char_resnet/msvd_annotation_{}.json'.format(global_step)
    output_file_name = '../all_results/results_cnn_chn_char_resnet/msvd_result_{}.json'.format(global_step)
